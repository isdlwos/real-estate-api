...
export class RentalLeasesService {
  ...
  async create(dto: CreateLeaseDto, userId: string): Promise<RentalLease> {
    ...
    if (existing) throw new ConflictException('Un bail actif existe déjà pour ce locataire');
    ...
    const full = await this.leaseRepo.findOne({ where: { id: lease.id }, relations: { payments: true, property: true } }) as RentalLease;
  ...
  private async generatePayments(lease: RentalLease): Promise<void> {
    ...
    const end = lease.endDate ? new Date(lease.endDate) : new Date(start.getFullYear(), start.getMonth() + 12, 1);
  ...
  async findOne(id: string, userId: string, userRole: Role): Promise<RentalLease> {
  ...
  async togglePayment(leaseId: string, paymentId: string, userId: string, userRole: Role): Promise<RentPayment> {
    const lease = await this.leaseRepo.findOneBy({ id: leaseId });
    if (!lease) throw new NotFoundException('Bail non trouvé');
    await this.checkAccess(lease, userId, userRole);

    const payment = await this.paymentRepo.findOneBy({ id: paymentId, leaseId });
  ...
  async updateNotes(id: string, body: { notes?: string; exitNotes?: string; entryNotes?: string }, userId: string, userRole: Role): Promise<RentalLease> {
  ...
  async terminate(id: string, userId: string, userRole: Role): Promise<RentalLease> {
    const lease = await this.leaseRepo.findOneBy({ id });
    if (!lease) throw new NotFoundException('Bail non trouvé');
    await this.checkAccess(lease, userId, userRole);
  ...
  async extend(id: string, months: number, userId: string, userRole: Role): Promise<RentalLease> {
    const lease = await this.leaseRepo.findOne({ where: { id }, relations: { payments: true } });
    if (!lease) throw new NotFoundException('Bail non trouvé');
    await this.checkAccess(lease, userId, userRole);
    if (lease.status === LeaseStatus.TERMINATED) throw new BadRequestException('Bail résilié');
    ...
    const lastMonth = lease.payments.reduce((max, p) => (p.month > max ? p.month : max), '0000-00');
    if (lastMonth !== '0000-00') {
      ...
      while (cur <= endDate) {
        ...
        newPayments.push({
          ...
          month,
          amount: lease.monthlyRent,
          status: cur < currentMonthStart ? PaymentStatus.LATE : PaymentStatus.PENDING,
        });
        cur.setMonth(cur.getMonth() + 1);
      }
  ...
  private async notifyTenantLeaseCreated(lease: RentalLease, agent: Agent): Promise<void> {
    if (!lease.tenantEmail) return;
    const agentWithUser = await this.agentRepo.findOne({ where: { id: agent.id }, relations: { user: true } });
  ...
  private async notifyAgentsLatePayments(): Promise<void> {
    ...
    for (const { agentId, lateCount } of lateByAgent) {
      ...
      if (agent?.user?.email) {
        await this.mailService.sendLatePaymentAlert({
          to: agent.user.email,
  ...
  async autoRenewLeases(): Promise<void> {
    ...
    const expiring = leases.filter((l) => l.endDate !== null && l.endDate <= today);
    ...
    for (const lease of expiring) {
      ...
      const newMonth = `${newEnd.getFullYear()}-${String(newEnd.getMonth() + 1).padStart(2, '0')}`;
      const existing = await this.paymentRepo.findOneBy({ leaseId: lease.id, month: newMonth });
      if (!existing) {
  ...
  async setAutoRenew(id: string, autoRenew: boolean, userId: string, userRole: Role): Promise<RentalLease> {
    const lease = await this.leaseRepo.findOneBy({ id });
    if (!lease) throw new NotFoundException('Bail non trouvé');
    await this.checkAccess(lease, userId, userRole);
  ...
  async generateReceiptPdf(leaseId: string, paymentId: string, userId: string, userRole: Role): Promise<Buffer> {
    const lease = await this.leaseRepo.findOne({
      where: { id: leaseId },
    ...
    if (!payment) throw new NotFoundException('Paiement non trouvé');
    if (payment.status !== PaymentStatus.PAID) throw new BadRequestException('Ce paiement n\'est pas encore encaissé');

    return this.buildReceiptPdf(lease, payment);
  }

  private buildReceiptPdf(lease: RentalLease, payment: RentPayment): Promise<Buffer> {
    return new Promise((resolve, reject) => {
      const doc = new PDFDocument({ size: 'A4', margin: 0, info: { Title: 'Quittance de loyer', Author: 'Prestige Immobilier' } });
      const chunks: Buffer[] = [];
      ...
      const paidStr  = payment.paidAt
        ? new Date(payment.paidAt).toLocaleDateString('fr-FR', { day: 'numeric', month: 'long', year: 'numeric' })
        : '—';
      const startStr = new Date(lease.startDate).toLocaleDateString('fr-FR', { day: 'numeric', month: 'long', year: 'numeric' });
      ...
      const box = (x: number, y: number, w: number, h: number, fill = light) => {
      ...
      doc.fillColor('white').fontSize(17).font('Helvetica-Bold')
        .text('PRESTIGE IMMOBILIER', L + P, curY + 16, { width: W / 2 - P * 2, lineBreak: false });
      doc.fillColor(gold).fontSize(9).font('Helvetica')
        .text('Agence immobilière · Dakar, Sénégal', L + P, curY + 38, { width: W / 2 - P * 2 });

      // Right side: document type
      doc.fillColor('white').fontSize(15).font('Helvetica-Bold')
        .text('QUITTANCE DE LOYER', L + W / 2, curY + 16, { width: W / 2 - P, align: 'right' });
      doc.fillColor(gold).fontSize(9).font('Helvetica')
        .text(`Réf. ${refNum}`, L + W / 2, curY + 39, { width: W / 2 - P, align: 'right' });
      ...
      const declText =
        ...
        `${amount.toLocaleString('fr-FR')} francs CFA, au titre du loyer du bien désigné ` +
        `ci-dessous pour la période du 1er ${MONTHS[mn - 1]} ${yr} ` +
        `au ${new Date(yr, mn, 0).getDate()} ${MONTHS[mn - 1]} ${yr}.`;

      const declH = doc.heightOfString(declText, { width: W - P * 2, lineGap: 4 }) + P * 2;
      box(L, curY, W, declH);
      ...
      doc.fillColor(navy).fontSize(9).font('Helvetica-Bold').text('Prestige Immobilier', L + P, cy, { width: cW - P * 2 });
      cy += 14;
      doc.fillColor(gray).fontSize(8).font('Helvetica').text('Dakar, Sénégal', L + P, cy, { width: cW - P * 2 });
      ...
      // ── 5. Bien loué ──────────────────────────────────────────────────────────
      const propTitle = property?.title ?? `Bail ${lease.id.split('-')[0].toUpperCase()}`;
      const propH = P + 11 + 14 + (addr ? 14 : 0) + P;
      box(L, curY, W, propH);
      cy = sectionLabel('BIEN LOUÉ', L + P, curY + P);
      doc.fillColor(navy).fontSize(9).font('Helvetica-Bold').text(propTitle, L + P, cy, { width: W - P * 2 });
      if (addr) {
        cy += 14;
        doc.fillColor(gray).fontSize(8).font('Helvetica').text(addr, L + P, cy, { width: W - P * 2 });
      }
      curY += propH + 12;

      // ── 6. Settlement table ───────────────────────────────────────────────────
      doc.fillColor(gold).fontSize(7).font('Helvetica-Bold').text('DÉTAIL DU RÈGLEMENT', L, curY);
      curY += 12;

      const rowH = 24;
      const tableRows: [string, string][] = [
        ['Loyer mensuel', `${amount.toLocaleString('fr-FR')} FCFA`],
        ['Charges', '0 FCFA'],
      ];
      for (const [label, value] of tableRows) {
        box(L, curY, W, rowH);
        doc.fillColor(gray).fontSize(9).font('Helvetica').text(label, L + P, curY + 8);
        doc.fillColor(navy).fontSize(9).font('Helvetica-Bold').text(value, L, curY + 8, { width: W - P, align: 'right' });
        curY += rowH;
      }
      // Total row — navy background
      const totH = 28;
      doc.rect(L, curY, W, totH).fill(navy).stroke(navy);
      doc.fillColor('white').fontSize(10).font('Helvetica-Bold').text('TOTAL REÇU', L + P, curY + 9);
      doc.fillColor(gold).fontSize(11).font('Helvetica-Bold')
        .text(`${amount.toLocaleString('fr-FR')} FCFA`, L, curY + 9, { width: W - P, align: 'right' });
      curY += totH + 16;
      ...
      const midX = L + W / 2;
      // Left cell
      doc.fillColor(gray).fontSize(7.5).font('Helvetica').text('DATE DE PAIEMENT', L + P, curY + P);
      doc.fillColor(navy).fontSize(9).font('Helvetica-Bold').text(paidStr, L + P, curY + P + 11, { width: W / 2 - P * 2 });
      // Vertical divider
      doc.moveTo(midX, curY + 8).lineTo(midX, curY + metaH - 8).stroke(border);
      // Right cell
      doc.fillColor(gray).fontSize(7.5).font('Helvetica').text('BAIL DEPUIS LE', midX + P, curY + P);
      doc.fillColor(navy).fontSize(9).font('Helvetica-Bold').text(startStr, midX + P, curY + P + 11, { width: W / 2 - P * 2 });
      curY += metaH + 16;
      ...
      doc.fillColor(gray).fontSize(7).font('Helvetica').text('Signature et cachet du bailleur', L + P, curY + P, { width: cW - P * 2 });
      // "Prestige Immobilier" faint
      doc.fillColor(silver).fontSize(8)
        .text('Prestige Immobilier', L + P, curY + sigH - 20, { width: cW - P * 2 });
      ...
      doc.fillColor(gray).fontSize(7.5).font('Helvetica').text(
        ...
        c2 + P, curY + P, { width: cW - P * 2, lineGap: 3 },
      );
      curY += sigH + 16;
      ...
      doc.fillColor(silver).fontSize(7).font('Helvetica')
        .text(`Prestige Immobilier · Dakar, Sénégal · Réf. ${refNum}`, L, footerY, { width: W, align: 'center' });

      doc.end();
    });
  }

  private async checkAccess(lease: RentalLease, userId: string, userRole: Role): Promise<void> {
